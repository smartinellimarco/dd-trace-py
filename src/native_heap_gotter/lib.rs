...
/// Report whether this cdylib was built with live-heap tracking (the
/// `ddheap:free` USDT + per-allocation retain flagging), so the free side can be
/// reconciled against allocations. This is a *compile-time* property of the
/// shipped artifact — alloc-only builds return `false`, live-heap builds return
/// `true` — and is NOT a runtime toggle. The Python activator surfaces it so the
/// profiler can report which native-heap mode is armed.
///
/// `cfg!(feature = "live-heap")` is our crate's own default feature, declared in
/// Cargo.toml as a pure forward to `libdd-profiling-heap-gotter/live-heap`,
/// which is exactly the condition under which the sampler compiled in
/// DD_HEAP_LIVE_TRACKING and emits `ddheap:free`. Keep this boolean in lockstep
/// with the Cargo feature so the runtime signal never disagrees with the ELF
/// `.note.stapsdt` reality.
///
/// # Safety
///
/// C ABI entry point with no arguments and no pointers; always safe to call.
